@echo off
REM 
REM ArcGIS Desktop 10.7 の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM デスクトップのインストール
echo ArcGIS Desktop 10.7のインストールを開始します
...
echo ArcGIS Desktop 10.7 のインストール... インストールにはしばらく時間がかかります...
...
REM 日本語パックのインストール
echo ArcGIS Desktop 10.7 日本語パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_Japanese\SetupFiles\Supplement\DesktopJapanese.msi" /norestart /passive /qb


REM 国内対応パックのインストール
echo ArcGIS Desktop 10.7 国内対応パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_JpnPack\JpnPack_Desktop10_7.msi" /norestart /passive /qb
...
echo ArcGIS Desktop 10.7 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Desktop 10.7 64-bit 国内対応パックのインストール ...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_JpnPack\JpnPack_Desktop10_7_Background.msi" /norestart /passive /qb

:64BITEND

echo ArcGIS Desktop 10.7のインストール終了
...
%windir%\System32\msiexec.exe /p "%curpath%\Desktop_Patches\ArcGIS-107-DT-NAFE-Patch.msp" /norestart /passive /qb

echo 02_Mosaic Dataset Compatibility Patch
%windir%\System32\msiexec.exe /p "%curpath%\Desktop_Patches\ArcGIS-107-DT-MDC-Patch.msp" /norestart /passive /qb
...
REM 2019/07/25 リリース時点
echo 01_Network Analyst Function Evaluator Patch
%windir%\System32\msiexec.exe /p "%curpath%\Desktop_Patches\ArcGIS-107-BGDT-NAFE-Patch.msp" /norestart /passive /qb
