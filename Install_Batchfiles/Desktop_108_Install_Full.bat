@echo off
REM 
REM ArcGIS Desktop 10.8 の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM デスクトップのインストール
echo ArcGIS Desktop 10.8のインストールを開始します
...
echo ArcGIS Desktop 10.8 のインストール... インストールにはしばらく時間がかかります...
REM Install ArcGIS Desktop 10.8
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_Japanese\SetupFiles\setup.msi" /norestart /passive /qb

REM 日本語パックのインストール
echo ArcGIS Desktop 10.8 日本語パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_Japanese\SetupFiles\Supplement\DesktopJapanese.msi" /norestart /passive /qb


REM 国内対応パックのインストール
echo ArcGIS Desktop 10.8 国内対応パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_JpnPack\JpnPack_Desktop10_8.msi" /norestart /passive /qb
...
echo ArcGIS Desktop 10.8 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Desktop 10.8 64-bit 国内対応パックのインストール ...
%windir%\System32\msiexec.exe /I "%curpath%\Desktop_JpnPack\JpnPack_Desktop10_8_Background.msi" /norestart /passive /qb

:64BITEND

echo ArcGIS Desktop 10.8 のインストール終了
...
%windir%\System32\msiexec.exe /p "%curpath%\Desktop_Patches\ArcGIS-108-DT-SSEGU-Patch.msp" /norestart /passive /qb
...
%windir%\System32\msiexec.exe /p "%curpath%\Desktop_Patches\ArcGIS-108-BGDT-SSEGU-Patch.msp" /norestart /passive /qb
