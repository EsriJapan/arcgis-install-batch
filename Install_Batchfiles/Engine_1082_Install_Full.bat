@echo off
REM 
REM ArcGIS Engine 10.8.2 の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM Engineのインストール
echo ArcGIS Engine 10.8.2 のインストールを開始します
...
echo ArcGIS Engine 10.8.2 のインストール... インストールにはしばらく時間がかかります...
REM Install ArcGIS Engine 10.8.2
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\setup.msi" /norestart /passive /qb

REM 日本語パックのインストール
echo ArcGIS Engine 10.8.2 日本語パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\Supplement\EngineJapanese.msi" /norestart /passive /qb


REM 国内対応パックのインストール
echo ArcGIS Engine 10.8.2 国内対応パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_JpnPack\JpnPack_Engine10_8_2.msi" /norestart /passive /qb
...
echo ArcGIS Engine 10.8.2 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Engine 10.8.2 のインストール終了
...
REM echo 01_xxxxxPatch
REM %windir%\System32\msiexec.exe /p "%curpath%\Engine_Patches\xxxxx.msp" /norestart /passive /qb
