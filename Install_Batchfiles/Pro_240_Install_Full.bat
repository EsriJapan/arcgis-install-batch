...
REM デスクトップのインストール
echo ArcGIS Pro 2.4 のインストールを開始します
...
echo ArcGIS Pro 2.4 本体のインストール... インストールにはしばらく時間がかかります...
...
echo ArcGIS Pro 2.4 日本語パック のインストール...
...
echo ArcGIS Pro 2.4 Patch 2 のインストール 
%windir%\System32\msiexec.exe /p "%curpath%\ArcGISPro_Patches\ArcGIS_Pro_242_170797.msp" /norestart /passive /qb

echo パッチのインストール終了

REM その他:ArcGIS Pro 2.0 用 ST_Geometry ライブラリ と ST_Raster ライブラリ
REM その他:日本用地図シンボルなどはバッチでは？
echo 注1)ArcGIS Pro 2.4 用 ST_Geometry ライブラリ と ST_Raster ライブラリ は必要に応じてインストールしてください
