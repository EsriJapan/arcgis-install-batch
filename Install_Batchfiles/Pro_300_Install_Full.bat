...
REM 
REM 参考： .NET SDK をインストールしている環境であれば、dotnet --list-runtimes でランタイムの一覧を取得してチェックする方法REM 参考： dotnet --list-runtimes でランタイムの一覧を取得してチェックする方法
...
echo.
echo -----------------------------------
echo ArcGIS Pro 3.0 の一括インストール の対象
echo -----------------------------------
echo ・ArcGIS Pro
...
REM デスクトップのインストール
echo ArcGIS Pro 3.0 のインストールを開始します
...
echo ArcGIS Pro 3.0 本体のインストール... インストールにはしばらく時間がかかります...
...
echo ArcGIS Pro 3.0 日本語パック のインストール...
...
echo ArcGIS Pro 3.0 Patch 1 のインストール...
%windir%\System32\msiexec.exe /p "%curpath%\ArcGISPro_Patches\ArcGIS_Pro_301_182232.msp" /norestart /passive /qb
...
REM echo ArcGIS Pro 3.0 Data Interoperability のインストール...
...
REM その他:日本用地図シンボルなど
echo.
echo 注1)ArcGIS Pro 3.0 用 データベースサポートファイル は必要に応じてMy Esriから入手してインストールしてください
