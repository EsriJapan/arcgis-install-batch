@echo off
REM 
REM ArcGIS Engine 10.8 の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM Engineのインストール
echo ArcGIS Engine 10.8 のインストールを開始します
...
echo ArcGIS Engine 10.8 のインストール... インストールにはしばらく時間がかかります...
REM Install ArcGIS Engine 10.8
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\setup.msi" /norestart /passive /qb

REM 日本語パックのインストール
echo ArcGIS Engine 10.8 日本語パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\Supplement\EngineJapanese.msi" /norestart /passive /qb


REM 国内対応パックのインストール
echo ArcGIS Engine 10.8 国内対応パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_JpnPack\JpnPack_Engine10_8.msi" /norestart /passive /qb
...
echo ArcGIS Engine 10.8 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Engine 10.8 のインストール終了
...
%windir%\System32\msiexec.exe /p "%curpath%\Engine_Patches\ArcGIS-108-E-SSEGU-Patch.msp" /norestart /passive /qb
...
%windir%\System32\msiexec.exe /p "%curpath%\Engine_Patches\ArcGIS-108-BGE-SSEGU-Patch.msp" /norestart /passive /qb
