@echo off
REM 
REM ArcGIS Engine 10.8.1 の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM Engineのインストール
echo ArcGIS Engine 10.8.1 のインストールを開始します
...
echo ArcGIS Engine 10.8.1 のインストール... インストールにはしばらく時間がかかります...
REM Install ArcGIS Engine 10.8.1
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\setup.msi" /norestart /passive /qb

REM 日本語パックのインストール
echo ArcGIS Engine 10.8.1 日本語パック のインストール...
...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_JpnPack\JpnPack_Engine10_8_1.msi" /norestart /passive /qb
...
echo ArcGIS Engine 10.8.1 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Engine 10.8.1 のインストール終了
