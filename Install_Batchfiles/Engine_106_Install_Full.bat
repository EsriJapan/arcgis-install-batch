@echo off
REM 
REM ArcGIS Engine 10.6の一括インストール
REM 
REM 以下を参考にして編集しました
...
REM Engineのインストール
echo ArcGIS Engine 10.6 のインストールを開始します
...
REM install MSXML6
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\Support\Msxml6\x64\msxml6_x64.msi" /norestart /passive /qb


:32BIT
echo MSXML6 のインストール...
REM install MSXML6
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\Support\Msxml6\x86\msxml6.msi" /norestart /passive /qb
...
echo ArcGIS Engine 10.6 のインストール... インストールにはしばらく時間がかかります...
...
REM 日本語パックのインストール
echo ArcGIS Engine 10.6 日本語パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_Japanese\SetupFiles\Supplement\EngineJapanese.msi" /norestart /passive /qb


REM 国内対応パックのインストール
echo ArcGIS Engine 10.6 国内対応パック のインストール...
%windir%\System32\msiexec.exe /I "%curpath%\Engine_JpnPack\JpnPack_Engine10_6.msi" /norestart /passive /qb
...
echo ArcGIS Engine 10.6 64-bit Background Geoprocessing のインストール...
...
echo ArcGIS Engine 10.6のインストール終了
...
%windir%\System32\msiexec.exe /p "%curpath%\Engine_Patches\ArcGIS-106-BGE-AES-Patch.msp" /norestart /passive /qb
